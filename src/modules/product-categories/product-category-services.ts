...
import {
    ...
    addReqBody,
    updateReqBody,
} from "./typings/product-category-type";
...
const add = async (newReqBody: addReqBody) => {
    ...
    return responseBuilder.createdSuccess(productCategoryMessage.ADD.SUCCESS, {
        id,
    });
};
...
const get = async (id: string | undefined, dataInfo: { title: string }) => {
    ...
    const { queryResult } = await productCategoryQueries.getAll(
        dataInfo.title as string
    );
    return responseBuilder.okSuccess(
        productCategoryMessage.GET.ALL,
        queryResult,
        {}
    );
};
...
const update = async (productCategoryId: IParamQuery, payload: addReqBody) => {
    ...
    const isExistId = await productCategoryQueries.getById(
        productCategoryId.id as string
    );
    if (!isExistId) {
    ...
    const data = await productCategoryQueries.update(
        productCategoryId.id as string,
        payload as updateReqBody
    );
    return responseBuilder.okSuccess(
        productCategoryMessage.UPDATE.SUCCESS,
        data
    );
};
...
const removeMultipleId = async (collectionId: Ids) => {
    ...
    const updateMultipleId = await productCategoryQueries.deleteMultipleIds(
        ids
    );
    ...
    return responseBuilder.okSuccess(
        productCategoryMessage.DELETE.SUCCESS,
        updateMultipleId
    );
};
