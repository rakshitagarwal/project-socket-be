...
export const storeOneMedia = (req: Request, res: Response, next: NextFunction) => {
    uploadSingle(req, res, function (err) {
        if (err instanceof multer.MulterError) {
            return res.status(400).json({ message: MESSAGES.MEDIA.MEDIA_SINGLE_INVALID});
        } else if (err) {
            return res.status(400).json({ message: MESSAGES.MEDIA.MEDIA_SINGLE_INVALID});
        }
        return next();
...
export const storeMultipleMedia = (req: Request, res: Response, next: NextFunction) => {
    uploadArray(req, res, function (err) {
        ...
        if (files.length as number === 0) return res.status(400).json({ message: MESSAGES.MEDIA.MEDIA_FILES_INVALID});
